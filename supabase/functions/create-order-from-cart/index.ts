// 檔案路徑: supabase/functions/create-order-from-cart/index.ts (Final Proactive Error Handling Version)
...
function createOrderEmailHtml(order, orderItems, address, shippingMethod, paymentMethod) {
    try {
        ...
        if (!Array.isArray(orderItems)) {
            throw new Error("orderItems 不是一個有效的陣列");
        }
        ...
        const itemsHtml = orderItems.map(item => {
            ...
            if (isNaN(priceAtOrder) || isNaN(quantity)) {
                console.warn("郵件範本發現異常商品資料:", item);
                return `
                    <tr style="border-bottom: 1px solid #eee;">
                        <td colspan="3" style="padding: 12px; color: #D9534F;">
                            ⚠️ ${variantName} 的金額資訊異常，請聯繫客服以確認訂單內容。
                        </td>
                    </tr>
                `;
            }
            ...
            return `
                <tr style="border-bottom: 1px solid #eee;">
                    ...
                    <td style="padding: 12px; text-align: center; vertical-align: top;">${quantity}</td>
        ...
        return `
            <div style="font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
                ...
                <div style="padding: 24px;">
                    ...
                    <p>感謝您的訂購。您的訂單 <strong>${order?.order_number || 'N/A'}</strong> 已經成功建立，我們將會盡快為您處理。</p>
                    <hr style="border: 0; border-top: 1px solid #eee; margin: 24px 0;">
                    <h3 style="font-size: 18px; margin-bottom: 16px;">訂單商品</h3>
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
                        <thead><tr style="background-color: #f7f7f7;"><th style="padding: 12px; text-align: left; border-bottom: 1px solid #ddd; font-size: 14px;">品名</th><th style="padding: 12px; text-align: center; border-bottom: 1px solid #ddd; font-size: 14px;">數量</th><th style="padding: 12px; text-align: right; border-bottom: 1px solid #ddd; font-size: 14px;">單價</th></tr></thead>
                        <tbody>${itemsHtml}</tbody>
                    </table>
                    <h3 style="font-size: 18px; margin-bottom: 16px;">費用明細</h3>
                    <table style="width: 100%; border-collapse: collapse;">
                        <tr><td style="padding: 6px 0;">商品小計</td><td style="padding: 6px 0; text-align: right;">${formatCurrency(order?.subtotal_amount || 0)}</td></tr>
                        ${(order?.coupon_discount || 0) > 0 ? `<tr><td style="padding: 6px 0; color: #D9534F;">折扣優惠</td><td style="padding: 6px 0; text-align: right; color: #D9534F;">- ${formatCurrency(order.coupon_discount)}</td></tr>` : ''}
                        <tr><td style="padding: 6px 0;">運費</td><td style="padding: 6px 0; text-align: right;">${formatCurrency(order?.shipping_fee || 0)}</td></tr>
                        <tr style="font-weight: bold; border-top: 1px solid #ccc; font-size: 1.2em;"><td style="padding: 12px 0;">總金額</td><td style="padding: 12px 0; text-align: right;">${formatCurrency(order?.total_amount || 0)}</td></tr>
                    </table>
                    <hr style="border: 0; border-top: 1px solid #eee; margin: 24px 0;">
        ...
        return "<p>您的訂單已確認，但郵件內容在產生時發生了一個預期外的錯誤，請聯繫客服。</p>";
    }
}
