// 檔案路徑: supabase/functions/create-order-from-cart/index.ts (Final Rich Response Version)
...
function createOrderEmailHtml(order, orderItems, address, shippingMethod, paymentMethod) {
    ...
    return `<div style="font-family: Arial, sans-serif; ...">...</div>`; // 邮件模板内容
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (backendSummary.total !== frontendValidationSummary.total) {
      return new Response(JSON.stringify({ error: { code: 'PRICE_MISMATCH', message: '訂單金額與當前優惠不符，請返回購物車重新確認。' } }), { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const { data: cartItems } = await supabaseAdmin.from('cart_items').select('*, product_variants(id, name)');
    ...
    const { data: newOrder, error: orderError } = await supabaseAdmin.from('orders').insert({
        ...
        payment_method: paymentMethod?.method_name || '未知', payment_status: 'pending',
