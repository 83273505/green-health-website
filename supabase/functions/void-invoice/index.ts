 ...
 * 2. [職責分離] 與 issue-invoice-manually 函式一樣，它作為一個安全的閘道，
 *          將請求轉發給 _shared/services/InvoiceService.ts 進行處理。
 * 3. [狀態檢查] 在執行作廢前，會先檢查目標發票的狀態，確保只有「已開立」的
 *          發票才能被作廢。
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!invoiceId || !reason) {
        throw new Error("請求中缺少必要的 'invoiceId' 或 'reason' 參數。");
    }
    if (reason.length > 20) {
        throw new Error("作廢原因長度不可超過 20 個字。");
    }
    ...
    if (invoiceToCheck.status !== 'issued') {
        throw new Error(`此發票的狀態為 "${invoiceToCheck.status}"，無法執行作廢操作。`);
    }
    ...
    console.error('[void-invoice] 函式發生未預期錯誤:', error.message);
    return new Response(
