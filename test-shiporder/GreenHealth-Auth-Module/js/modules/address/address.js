// 檔案路徑: js/modules/address/address.js
...
async function handleFormSubmit(event) {
    ...
    const { error } = addressId 
        ? await supabase.from(TABLE_NAMES.ADDRESSES).update(formData).eq('id', addressId)
        : await supabase.from(TABLE_NAMES.ADDRESSES).insert(formData);
...
async function handleDeleteAddress(addressId) {
    ...
    const { error } = await supabase.from(TABLE_NAMES.ADDRESSES).delete().eq('id', addressId);
    if (error) {
...
async function handleSetDefaultAddress(addressId) {
    try {
        ...
        const { data, error } = await supabase.functions.invoke('set-default-address', { body: { addressId: addressId } });
        if (error) throw error;
        if (data.error) throw new Error(data.error);
        await fetchAddresses();
        showNotification('預設地址已成功更新！', 'success');
    } catch (error) {
        console.error('透過 Edge Function 設定預設地址時發生錯誤:', error);
