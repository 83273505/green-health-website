...
async function handleOrderSelection(orderId) {
    ...
    try {
        const { data: items, error } = await supabase.functions.invoke(FUNCTION_NAMES.GET_ORDER_DETAILS, { body: { orderId: selectedOrderId } });
    ...
    populateCarrierSelector(selectedOrder.shipping_rates?.method_name);
}
...
async function populateCarrierSelector(defaultCarrier) {
    if (shippingRatesCache.length === 0) {
        ...
        if (error) { console.error("讀取運送方式失敗", error); return; }
...
async function handlePaymentConfirmation(event) {
    ...
    try {
        const { data, error } = await supabase.functions.invoke(FUNCTION_NAMES.MARK_ORDER_AS_PAID, {
...
async function handleSearchShippedOrders(event) {
    ...
    try {
        const { data: results, error } = await supabase.functions.invoke(FUNCTION_NAMES.SEARCH_SHIPPED_ORDERS, {
            body: filteredParams
        });
        if (error) throw error;
...
async function handleResendNotification(orderId, orderNumber) {
    ...
    try {
        const { data, error } = await supabase.functions.invoke(FUNCTION_NAMES.RESEND_SHIPPED_NOTIFICATION, {
            body: { orderId }
        });
        if (error) throw error;
        if (data.error) throw new Error(data.error);
