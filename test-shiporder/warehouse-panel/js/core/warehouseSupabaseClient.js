    ...
    clientPromise = (async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`無法從伺- 服器獲取 Supabase 設定: ${response.status} ${errorText}`);
            }
            ...
            console.log('✅ Warehouse Panel Supabase Client 初始化成功');
            ...
            console.error('❌ Warehouse Panel Supabase Client 初始化失敗:', error);
            alert("倉庫後台系統初始化失敗，請聯絡管理員。");
            // 失敗後重設 promise，允許重試
            clientPromise = null;
            throw error;
        }
