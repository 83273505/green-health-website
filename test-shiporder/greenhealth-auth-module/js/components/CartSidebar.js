// ==============================================================================
// 檔案路徑: js/components/CartSidebar.js
// ------------------------------------------------------------------------------
// 【此為完整檔案，可直接覆蓋 - v20.0 防禦性渲染版】
// ==============================================================================
...
// --- 模組內部變數 ---
// 我們不再在模組頂層緩存 DOM 元素，以避免時序問題
let sidebarElement, overlayElement; 
...
function render(state) {
    // 【核心修改】將 DOM 元素獲取移至 render 函式內部
    // 這確保了每次渲染時，我們都能獲取到最新、最準確的 DOM 元素參照。
    const itemListElement = document.getElementById('sidebar-item-list');
    const subtotalElement = document.getElementById('sidebar-subtotal-price');
    ...
    const couponDiscountElement = document.getElementById('sidebar-coupon-discount');
    const shippingFeeElement = document.getElementById('sidebar-shipping-fee');
    const totalElement = document.getElementById('sidebar-total-price');
    const shippingSelectorContainer = document.getElementById('shipping-selector-container');

    if (!itemListElement) return; // 如果核心元素不存在，則不進行渲染
    ...
    if (state.items.length === 0) {
        itemListElement.innerHTML = `
            <div class="empty-cart-container">
                ...
                <a href="/GreenHealth-Auth-Module/products.html" class="continue-shopping-btn">繼續購物</a>
            </div>
        `;
    } else {
    ...
    if (shippingSelectorContainer) {
        if (!state.isReadyForRender) {
            // 情況 A: CartService 尚未完全初始化，顯示載入中
            shippingSelectorContainer.style.display = 'block';
            shippingSelectorContainer.innerHTML = `<p style="font-size: 0.9rem; color: #666;">正在載入運送方式...</p>`;
        } else if (state.items.length > 0 && state.availableShippingMethods.length > 0) {
            // 情況 B: 已就緒，且有商品和運送方式，渲染完整下拉選單
            shippingSelectorContainer.style.display = 'block';
...
export const CartSidebar = {
        ...
        const sidebarHtml = `
            ...
            <div id="cart-sidebar" class="hidden">
                ...
                <div class="sidebar-footer">
                    ...
                    <div id="shipping-selector-container" class="shipping-section"></div>
                    ...
                    <a href="/GreenHealth-Auth-Module/checkout.html" id="checkout-btn" class="checkout-btn disabled">前往結帳</a>
                </div>
            </div>`;
    ...
    open() { if (!sidebarElement) this.init(); sidebarElement.classList.add('open'); sidebarElement.classList.remove('hidden'); if (overlayElement) { overlayElement.classList.add('open'); overlayElement.classList.remove('hidden'); } },
    ...
    toggle() { if (!sidebarElement) this.init(); if (sidebarElement?.classList.contains('open')) { this.close(); } else { this.open(); } }
};