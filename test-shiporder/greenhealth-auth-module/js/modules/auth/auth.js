...
function initLoginPage() {
    ...
    // 【核心重構】Email/密碼登入流程 - 增加完整的錯誤處理和使用者體驗優化
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (!data.user) {
                    throw new Error('登入成功但無法取得使用者資訊');
                }
                ...
                // 登入成功後，強制刷新一次購物車服務，確保狀態同步
                await CartService.forceReinit();

                const successMessage = mergeSuccessful ? 
                    '登入成功！已為您合併訪客購物車，正在導向會員主頁...' : 
                    '登入成功！正在將您導向會員主頁...';
    ...
    // 【核心修改】移除 onAuthStateChange，因為所有登入流程都已改為手動跳轉
}

function initAuthCallbackPage() {
    supabase.auth.onAuthStateChange(async (event, session) => {
        if (event === 'SIGNED_IN' || (event === 'INITIAL_SESSION' && session)) {
            console.log('🔐 [AuthCallback] 偵測到 OAuth 登入成功事件。');
            ...
            if (anonymousUserId && anonymousUserId !== currentUserId) {
                console.log(`🔗 [AuthCallback] 從 URL 發現匿名購物車需要轉移: ${anonymousUserId} -> ${currentUserId}`);
                try {
                ...
                console.log('🔐 [AuthCallback] OAuth 登入無需資料轉移，直接完成。');
            }
            ...
            console.log('🔄 [AuthCallback] 正在重新導向至儀表板...');
...
function initUpdatePasswordPage() {
    ...
    if (!updatePasswordForm) {
        console.error('在 update-password 頁面找不到 #update-password-form 元素。');
        return;
    }
...
async function transferAnonymousUserData(anonymousUserId, currentUserId) {
    ...
    try {
        ...
        console.log(`[Transfer] 發現 ${anonymousCart.cart_items.length} 個商品需要轉移。`);

        let { data: currentCart } = await supabase
            .from('carts')
        ...
        if (itemsToInsert.length > 0) {
            await supabase.from('cart_items').insert(itemsToInsert).throwOnError();
        }
...
async function cleanupAnonymousUserData(anonymousUserId, anonymousCartId) {
    try {
        if (anonymousCartId) {
            await supabase.from('cart_items').delete().eq('cart_id', anonymousCartId);
