 ...
 *              不再依賴外部注入 supabase 客戶端。
 * @version v42.2
 * 
 * @update v42.2 - [ANONYMOUS USER PERSISTENCE FIX]
 * 1. [核心修正] 重構了 init() 函式的初始化流程。現在，當從 localStorage 恢復
 *          狀態時，會立即嘗試使用儲存的匿名使用者 token 呼叫 supabase.auth.setSession()。
 * 2. [原理] 此修正強制 supabase-js 客戶端恢復到上一次的匿名會話，而不是建立
 *          一個新的匿名身份，從而確保了前端請求的 auth.uid() 與資料庫中 carts.user_id
 *          的絕對一致性，徹底解決了因身份不匹配導致的 RLS 靜默失敗問題。
 * 3. [新增] 在 localStorage 中增加了對 'anonymous_token' 的儲存與讀取。
 * 4. [同步] 調整了處理 'get-or-create-cart' 回應的邏輯，以正確儲存後端
 *          返回的匿名 token。
 * 5. [保留] 完整保留了 v41.2 的超時處理等所有網路健壯性功能。
 */
...
const INVOKE_TIMEOUT = 10000; // 10 秒
...
async function invokeWithTimeout(functionName, options = {}) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            throw new Error(`對 ${functionName} 的請求已逾時 (${INVOKE_TIMEOUT / 1000} 秒)。`);
        }
        throw error;
    }
}

// [v42.2 修正] _restoreStateFromLocalStorage 現在只負責讀取資料
function _restoreStateFromLocalStorage() {
    try {
        ...
        const anonymousToken = localStorage.getItem('anonymous_token'); // 新增
...
async function _recalculateCart(payload) {
    ...
    _notify();
    try {
        const { data: snapshot, error } = await invokeWithTimeout('recalculate-cart', {
        ...
        const userMessage = error.message.includes('逾時') 
            ? '購物車連線逾時，請檢查您的網路環境後重試。' 
            : '購物車更新失敗，請重試。';
...
export const CartService = {
            ...
            try {
                ...
                if (restored && anonymousToken && anonymousUserId) {
                    ...
                    const { error } = await _supabase.auth.setSession({
                        ...
                        refresh_token: 'dummy_refresh_token' // 匿名使用者 token 不會過期，refresh token 非必需
                    });
                    if (error) {
                        console.warn('恢復匿名 Session 失敗，將重新獲取:', error.message);
                        this.clearCartAndState(); // 清理舊的無效狀態
                    }
                }
                ...
                if (!_state.cartId) {
                    ...
                    if (data.isAnonymous && data.userId && data.token) {
                        ...
                        localStorage.setItem('anonymous_token', data.token); // 儲存匿名 token
                    } else {
                ...
                const userMessage = error.message.includes('逾時')
                    ? '初始化購物車失敗：連線逾時，請重新整理頁面。'
                    : `初始化購物車失敗：${error.message}`;
                showNotification(userMessage, 'error', 'notification-message');
                _initPromise = null;
                _state.isReadyForRender = false;
                _notify();
    ...
    clearCartAndState() {
        try {
            ...
            localStorage.removeItem('anonymous_user_id');
            localStorage.removeItem('anonymous_token'); // 新增
            _state = {
