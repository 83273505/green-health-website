...
更新日誌 (Changelog)：
v44.1.1 (2025-09-13)：修正案交付，確保檔案的物理完整性。
v44.1 (2025-09-13)：完成對 CartService 的依賴校準。
*/
...
y
addr.city
{addr.district}${addr.street_address.substring(0, 10)}...}</option>
).join('');
...
if (userAddresses.length > 0) {
populateAddressForm(userAddresses);
}
} catch (error) {
...
case 'business':
if (!/^\d{8}
/.test(invoiceOptions.donation_code)) {
showNotification('愛心碼格式不正確，應為 3-7 位數字。', 'error', 'notification-message');
return false;
}
break;
case 'cloud':
if (invoiceOptions.carrier_type === 'mobile' && !/^/[A-Z0-9+-.]{7}
/.test(invoiceOptions.carrier_number)) {
